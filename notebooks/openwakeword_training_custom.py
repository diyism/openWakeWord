...
# --- 定义源文件模式 ---
positive_files_pattern = os.path.join(dir_cut_syl, "a.*.wav") # <<< 修改这里
...
# === 创建虚拟负样本文件以避免 StopIteration ===
# 仅当存在正样本时才创建（避免不必要的操作和潜在错误）
# 需要确保 scipy 和 numpy 已导入
import numpy as np
import scipy.io.wavfile

if num_pos_train > 0 or num_pos_test > 0: # 如果有任何正样本被复制
    # 创建训练集虚拟文件
    dummy_neg_train_file_path = os.path.join(neg_train_dir, "dummy_negative.wav")
    print(f"\nCreating dummy negative file in {neg_train_dir} to prevent errors...") # Removed trailing ' during feature computation'
    try:
        # 创建一个短的 (例如 50ms) 静音 16kHz WAV 文件
        sampling_rate = 16000
        duration_ms = 50
        ...
        print(f"Dummy file created: {dummy_neg_train_file_path}")
    except Exception as e:
        print(f"Error creating dummy negative train file: {e}")

    # === 新增：创建测试集虚拟文件 ===
    dummy_neg_test_file_path = os.path.join(neg_test_dir, "dummy_negative.wav")
    print(f"\nCreating dummy negative file in {neg_test_dir} to prevent errors...")
    try:
        # 使用上面生成的相同静音数据
        scipy.io.wavfile.write(dummy_neg_test_file_path, sampling_rate, silence)
        print(f"Dummy file created: {dummy_neg_test_file_path}")
    except Exception as e:
        print(f"Error creating dummy negative test file: {e}")
    # === 结束新增 ===

else:
     print("\nSkipping dummy negative file creation as no positive files were copied.")
# === 结束新增 ===
